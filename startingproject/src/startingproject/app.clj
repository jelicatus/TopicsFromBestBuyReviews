...
(en/deftemplate fill-products 
  ...
  (en/clone-for[product  productss] 
     ...
     [:p.prices] (en/content (str "regularPrice " (product :regularPrice) " "))
     [:p.long-description] (en/content (str (product :longDescription) " "))                        
     [:h1.url] (en/content (str (product :url)))
     )
                             )
...
(defn searchforproducts [product]
   (reset! products
    {:data   (((client/get (str "http://api.remix.bestbuy.com/v1/products(name=" product "*&categoryPath.name=%22Cell%20Phones%22)?show=name,image,manufacturer,regularPrice,onSale,percentSavings,salePrice,longDescription&pageSize=30&page=30&apiKey=5qxg5sxjbbxa9maxpfrqvqjw&format=json")
           {:form-params "body"
            :content-type :json
            :as :json}
            ) :body) :products) } )
...
(defroutes app*
  ...
  (POST "/searchforproducts" [product]
        ...
        ;   (str product) (:product (:params product))
